...
module.exports = async (req, res) => {
  if (req.method !== 'POST') {
    ...
    return res.status(200).json({ status: 'ok', message: 'Bot2 (Particle Blast) Webhook is running' });
  }
    ...
    if (update.message) {
      ...
      if (text.startsWith('/start') || text.startsWith('/play')) {
        ...
        await telegramAPI('sendMessage', {
          chat_id: chatId,
          text: '💥 ' + (text.startsWith('/start') ? '欢迎 ' + username + ' 来到 Particle Blast！' : 'Particle Blast') + '\n\n点击下方按钮开始游戏：',
          reply_markup: {
            inline_keyboard: [
